...
const queryObj = {
  ...
  async function setArtileList() {
    ...
    // 1.3 展示到指定的标签结构中  item.cover.type判断是否有图片，没有就给了默认图片
    // console.log( res)
...
async function setChannleList() {
  ...
  document.querySelectorAll('.form-check-input').forEach(radio => { //获取到所有form-check-input标签的样式
    radio.addEventListener('change', e => {//给所有form-check-input标签的样式都遍历添加点击事件
...
document.querySelector('.next').addEventListener('click', e => {
    // 当前页码小于最大页码数  计算总页码：Math.ceil向上取整
 ...
 */
// 4.2 点击删除时，获取文章 id
//为什么绑定的是父标签的art-list？，而不是绑定本身的？
//答：因为我们<td>的内容是动态生成的，所以点击事件要绑定父标签
document.querySelector('.art-list').addEventListener('click', async e => {
    // 判断点击的是删除元素 e.target：获取子标签的内容
    if (e.target.classList.contains('del')) { 
        //e.target.parentNode：从子标签为参考来找父标签的data-id dataset.id对应的就是data-id
    ...
    // 4.5 删除最后一页的最后一条，需要自动向前翻页
    const children = document.querySelector('.art-list').children  //获取当前tbody标签下的有多少个tr标签  children.length获取tr有几个
